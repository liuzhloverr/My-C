...
{   //argv[1] 是code argv[2]是起始时间，argv[3]是终止时间，argv[4]是时间间隔
	...
	pFile = fopen("/Users/liuzh/Desktop/reptile/output/data/timeLine.txt", "ab+");
  ...
  {    //第一个时间间隔，要在Timelist中找最后一个比dwStartTime小的值
    ...
    //printf("%s %f\n","开盘价",dwOpenPrice );
		...
		if (dwtmp1 != dwtmp2)
		{
		    ...
		    fprintf (pFile, "%f\t %f\t %f\t %f\t %f\t %d\n",
		    	dwOpenPrice,dwClosePrice,dwHighPrice,dwLowPrice,dwAmount,dwVolume);
